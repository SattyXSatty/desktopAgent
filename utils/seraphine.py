...
def run_seraphine_grouping(merged_detections, config, image_path=None):
    ...
    if image_path and enhanced_analysis and 'bbox_processor' in enhanced_analysis:
        ...
        if hasattr(bbox_processor, 'final_groups') and bbox_processor.final_groups:
            ...
            try:
                # ✅ ROBUST SYNC-TO-ASYNC: Handle cases where loop is already running (e.g. MCP)
                try:
                    loop = asyncio.get_running_loop()
                    import concurrent.futures
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        # Use lambda to ensure correct coroutine object handling
                        future = executor.submit(lambda: asyncio.run(analyze_supergroups_with_gemini(visualization_path)))
...
def process_image_sync(image_path, config_path=None):
    ...
    while attempt < max_attempts:
        attempt += 1
        try:
            # ✅ ROBUST ASYNC BRIDGE: Handle cases where loop is already running (e.g. MCP)
            try:
                loop = asyncio.get_running_loop()
                # Run in thread pool to avoid "asyncio.run() cannot be called from a running event loop"
                import concurrent.futures
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    return executor.submit(lambda: asyncio.run(process_image(image_path, config_path))).result()
            except RuntimeError:
                # No running loop, asyncio.run is fine
